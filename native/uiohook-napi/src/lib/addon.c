...
void dispatch_proc(uiohook_event* const event) {
  ...
  // LOCAL PATCH -- see native/PROVENANCE.md.
  //
  // Movement, drags and the wheel are dropped here, on the hook thread, before anything
  // is allocated or handed to JavaScript. This client binds keys and the two extra mouse
  // buttons and nothing else, but `WH_MOUSE_LL` reports every motion: with a 1000 Hz
  // mouse that is a thousand allocations, a thousand JavaScript objects and a thousand
  // event-loop wakeups a second, in the same process that reads the game's memory.
  //
  // It also means the app never receives a cursor position at all, which is a better
  // thing to be able to say about a process holding a global input hook.
