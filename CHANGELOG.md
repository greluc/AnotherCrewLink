...
- **The installer is now built by this project rather than by the packaging tool.** You
  ...
  The moment to find out whether an installer handles an update correctly is not the
  moment several hundred machines run it at once, so it carried an ordinary release first.
  If anything about installing or updating looks different to you, that is worth
  reporting — it is the one part of this release that is genuinely new.
