...
**All five mechanisms are in as of 2026-08-24.** The repository is ours — the
client was moved off `OhMyGuus/BetterCrewlink-Offsets` and onto
`greluc/AnotherCrewlink-Offsets` — and the other four followed in H2:

| Mechanism | Where it lives |
| --- | --- |
| The mirror, and a sync that proposes rather than pushes | `AnotherCrewlink-Offsets`: `.github/workflows/sync-upstream.yml`, branch protection on `main` |
| A bundle envelope: `bundle_version`, `min_client_version`, `upstream_commit` | `lookup.json`, enforced in `offsetsValidator.ts` |
| The client reads the mirror, never a third party's branch HEAD | `offsetStore.ts` |
| An embedded bundle as the floor | `embeddedOffsets.ts`, generated by `scripts/embed-offsets.mjs` |
| Structural validation on every load, and a write-side prologue check | `offsetsValidator.ts`, `GameReader.inspectDetourSite` |
| "Reset offsets to embedded" | Settings → Troubleshooting, `IpcHandlerMessages.RESET_OFFSETS` |

What that buys, and what it does not, is worth keeping separate. It closes
following an unpinned branch of a repository nobody here controls, a bundle that
nothing looked at between the fetch and its use, a cache that was validated only
at download time, a fetch failure that was fatal, and a detour written into bytes
this client had never inspected. It does **not** close a structurally perfect
bundle carrying wrong numbers: that reads the wrong field, and nothing in the
client distinguishes it from a game update. Review on the mirror is what stands
there, which is why branch protection is part of the deliverable rather than a
settings afterthought.

One duty arrives with the fork and does not go away: keeping it in sync. A mirror
that lags upstream after an Among Us patch does not expose users to an attacker,
it locks them out of the game, and that failure is on us rather than on someone
else's schedule. The floor is what keeps that from being fatal — a client whose
mirror is unreachable starts on the bundle compiled into it and says so.
...
and takes about a second. It is automated: `purge-cdn.yml` on the mirror runs on
every push to `main` that touches `lookup.json` or `offsets/`, and warns rather
than fails when a purge does not answer — a stale fallback is worth noticing, not
worth blocking a release the primary host is already serving correctly.
